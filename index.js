...
const dataFile = path.join(process.env.RENDER_DISK_MOUNT || __dirname, 'assets.json');
...
async function loadAssets() {
  try { return new Map(JSON.parse(await fs.readFile(dataFile, 'utf8'))); }
  catch { return new Map(); }
}
async function saveAssets(assets) {
  try { await fs.writeFile(dataFile, JSON.stringify([...assets], null, 2)); }
...
.print-modal-close {
  ...
  justify-content: center; font-size: 16px; color: #6b7280; line-height: 1;
}
...
.print-opt-btn {
  border: 2px solid #e5e7eb; border-radius: 7px; padding: 8px 6px;
  text-align: center; cursor: pointer; transition: all 0.12s; background: #fff;
  font-family: inherit;
}
...
.ami-label-company .co-sub { font-size: 9px; color: #555; margin-top: 2px; letter-spacing: 0.06em; }
...
.ami-field-row.cols-1 { grid-template-columns: 1fr; }
  ...
  <div class="print-modal">
    <div class="print-modal-header">
      <div class="print-modal-title">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display:inline-block;vertical-align:-3px;margin-right:6px;"><path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
        Print Settings
      </div>
    ...
    <div class="print-modal-body">
      ...
      <div class="print-option-group">
        ...
        <div class="print-option-grid" id="perPageGrid">
          <button class="print-opt-btn" data-perpg="1" onclick="setPerPage(1)"><div class="opt-num">1</div><div class="opt-lbl">1&#x00D7;1</div></button>
          <button class="print-opt-btn selected" data-perpg="4" onclick="setPerPage(4)"><div class="opt-num">4</div><div class="opt-lbl">2&#x00D7;2</div></button>
          <button class="print-opt-btn" data-perpg="6" onclick="setPerPage(6)"><div class="opt-num">6</div><div class="opt-lbl">2&#x00D7;3</div></button>
          <button class="print-opt-btn" data-perpg="8" onclick="setPerPage(8)"><div class="opt-num">8</div><div class="opt-lbl">2&#x00D7;4</div></button>
        </div>
      </div>
      <div class="print-option-group">
        ...
        <div id="customCopiesWrap" style="display:none;margin-top:8px;">
          <input type="number" id="customCopiesInput" min="1" max="200" value="10" placeholder="Enter copies (1-200)" oninput="updatePreview()">
        </div>
      </div>
      <div class="print-option-group">
      ...
      <div class="print-option-group">
        ...
        <div style="border:1px solid #e5e7eb;border-radius:8px;padding:0 12px;">
          <div class="print-toggle-row">
            <div class="print-toggle-info"><div class="ptl">Show QR Code</div><div class="pts">Include QR code image on each label</div></div>
          ...
          <div class="print-toggle-row">
            <div class="print-toggle-info"><div class="ptl">Show Company Header</div><div class="pts">Print company name and AMI number at top</div></div>
          ...
          <div class="print-toggle-row">
            <div class="print-toggle-info"><div class="ptl">Show Border</div><div class="pts">Bold outer border on each label</div></div>
...
(function() {
  ...
  var LAYOUTS = { 1:[1,1], 2:[2,1], 4:[2,2], 6:[2,3], 8:[2,4] };

  window.setPerPage = function(n) {
    state.perPage = n;
    var l = LAYOUTS[n] || [2,2];
    state.cols = l[0]; state.rows = l[1];
    document.querySelectorAll('#perPageGrid .print-opt-btn').forEach(function(b) {
      b.classList.toggle('selected', parseInt(b.dataset.perpg) === n);
    });
    updatePreview();
  };

  window.setCopies = function(v) {
    state.copies = v;
    document.getElementById('customCopiesWrap').style.display = (v === 'custom') ? 'block' : 'none';
    document.querySelectorAll('#copiesGrid .print-opt-btn').forEach(function(b) {
      b.classList.toggle('selected', b.dataset.copies === String(v));
    });
    updatePreview();
  };
  ...
  window.setPageSize = function(v) {
    state.pageSize = v;
    document.querySelectorAll('#pageSizeGrid .print-opt-btn').forEach(function(b) {
      b.classList.toggle('selected', b.dataset.pgsz === v);
    });
    updatePreview();
  };
  ...
  window.updatePreview = function() {
    ...
    var totalCopies;
    if (state.copies === 'auto') totalCopies = state.perPage;
    else if (state.copies === 'custom') {
      totalCopies = parseInt(document.getElementById('customCopiesInput').value) || state.perPage;
    } else totalCopies = parseInt(state.copies);

    var pages = Math.ceil(totalCopies / state.perPage);
    ...
    info.textContent = totalCopies + ' label' + (totalCopies !== 1 ? 's' : '') + ' \u2022 ' + pages + ' page' + (pages !== 1 ? 's' : '') + ' \u2022 ' + state.cols + '\u00D7' + state.rows + ' layout';
  };
  ...
  function buildLabelHTML(a, qrSrc, showQR, showCompany, border, inner) {
    var monthNum = ('0' + (a.month || '03')).slice(-2);
    return '<div style="border:' + border + ';font-family:Courier New,monospace;background:#fff;color:#111;page-break-inside:avoid;break-inside:avoid;box-sizing:border-box;">' +
      (showCompany ?
        '<div style="display:flex;align-items:stretch;border-bottom:' + inner + ';">' +
          '<div style="flex:1;padding:5px 10px;border-right:' + inner + ';">' +
            '<div style="font-size:7px;font-weight:bold;color:#555;letter-spacing:0.08em;">COMPANY</div>' +
            '<div style="font-size:10px;font-weight:bold;">' + escH(a.companyName.toUpperCase()) + '</div>' +
          '</div>' +
          '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:5px 10px;border-right:' + inner + ';min-width:56px;text-align:center;">' +
            '<div style="font-size:7px;font-weight:bold;letter-spacing:0.1em;">* AMI *</div>' +
            '<div style="font-size:19px;font-weight:bold;line-height:1;">' + monthNum + '</div>' +
          '</div>' +
          '<div style="display:flex;align-items:center;justify-content:center;padding:5px 9px;">' +
            '<div style="border:' + inner + ';padding:3px 5px;font-size:8px;font-weight:bold;line-height:1.3;text-align:center;">ROHS 2<br>FREE</div>' +
          '</div>' +
        '</div>' : '') +
      '<div style="display:grid;grid-template-columns:1fr 1fr;border-bottom:' + inner + ';">' +
        '<div style="padding:4px 10px;"><div style="font-size:7px;font-weight:bold;letter-spacing:0.1em;color:#555;text-transform:uppercase;">Part No.</div><div style="font-size:11px;font-weight:bold;font-family:Courier New,monospace;">' + escH(a.partNo) + '</div></div>' +
        '<div style="padding:4px 10px;border-left:' + inner + ';"><div style="font-size:7px;font-weight:bold;letter-spacing:0.1em;color:#555;text-transform:uppercase;">Part Name</div><div style="font-size:11px;font-weight:bold;">' + escH(a.partName) + '</div></div>' +
      '</div>' +
      '<div style="display:grid;grid-template-columns:1fr 1fr;border-bottom:' + inner + ';">' +
        '<div style="padding:4px 10px;"><div style="font-size:7px;font-weight:bold;letter-spacing:0.1em;color:#555;text-transform:uppercase;">Size</div><div style="font-size:11px;font-weight:bold;">' + escH(a.size || '\u2014') + '</div></div>' +
        '<div style="padding:4px 10px;border-left:' + inner + ';"><div style="font-size:7px;font-weight:bold;letter-spacing:0.1em;color:#555;text-transform:uppercase;">Lot No.</div><div style="font-size:11px;font-weight:bold;font-family:Courier New,monospace;">' + escH(a.lotNo) + '</div></div>' +
      '</div>' +
      '<div style="display:grid;grid-template-columns:1fr 1fr;border-bottom:' + inner + ';">' +
        '<div style="padding:4px 10px;"><div style="font-size:7px;font-weight:bold;letter-spacing:0.1em;color:#555;text-transform:uppercase;">Quantity</div><div style="font-size:13px;font-weight:bold;">' + escH(a.quantity) + '</div></div>' +
        '<div style="padding:4px 10px;border-left:' + inner + ';"><div style="font-size:7px;font-weight:bold;letter-spacing:0.1em;color:#555;text-transform:uppercase;">Packer</div><div style="font-size:11px;font-weight:bold;">' + escH(a.packer) + '</div></div>' +
      '</div>' +
      '<div style="display:flex;align-items:stretch;min-height:76px;">' +
        (showQR ?
          '<div style="border-right:' + inner + ';padding:6px 8px;display:flex;flex-direction:column;align-items:center;justify-content:center;">' +
            '<div style="font-size:6px;font-weight:bold;letter-spacing:0.08em;color:#555;margin-bottom:3px;">QR CODE</div>' +
            '<img src="' + qrSrc + '" style="width:58px;height:58px;display:block;">' +
            '<div style="font-size:6px;color:#888;margin-top:2px;">SCAN FOR DETAILS</div>' +
          '</div>' : '') +
        '<div style="flex:1;padding:8px 10px;display:flex;flex-direction:column;justify-content:center;gap:3px;">' +
          '<div style="font-size:7px;font-weight:bold;letter-spacing:0.1em;color:#555;">WO NO.</div>' +
          '<div style="font-size:10px;font-weight:bold;font-family:Courier New,monospace;">' + escH(a.woNo || '\u2014') + '</div>' +
        '</div>' +
      '</div>' +
    '</div>';
  }
  ...
  document.getElementById('confirmPrintBtn').addEventListener('click', function() {
    ...
    if (state.copies === 'auto') totalCopies = state.perPage;
      ...
      if (totalCopies > 200) totalCopies = 200;
    } else totalCopies = parseInt(state.copies);

    var borderStyle = state.showBorder ? '2.5px solid #111' : '1px solid #ccc';
    var innerBorder = state.showBorder ? '1.5px solid #111' : '1px solid #ddd';

    var pageSizeCSS = {
      'A4':     'size: A4 portrait',
      'A4L':    'size: A4 landscape',
      'A5':     'size: A5 portrait',
      'A5L':    'size: A5 landscape',
    ...
    styleEl.innerHTML = [
      ...
      '    grid-template-columns: repeat(' + state.cols + ', 1fr) !important;',
      '    gap: 6mm !important;',
...
<aside class="sidebar">
  <div class="sidebar-brand">
    ...
    <div>
      <div class="brand-name">AMI QR System</div>
      <div class="brand-sub">Production Portal</div>
    </div>
  </div>
  ...
  <div class="sidebar-footer">
    <a href="/logout" class="nav-link red">${I.logout} Sign Out</a>
  </div>
</aside>`;
...
app.get('/', (req, res) => {
  if (!isAuthenticated(req)) {
    return res.send(`${HEAD('Sign In')}<body>
    <div class="auth-wrap">
          ...
          <div class="card-body">
            <form action="/login" method="POST">
              <div class="form-1">
                ...
                <input type="text" name="id" placeholder="Enter admin ID" required autocomplete="username">
              </div>
  ...
  ${LAYOUT(`
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Welcome back &mdash; AMI QR System</p>
      </div>
    ...
    <div class="stats-row">
      <div class="stat-box">
        ...
        <div class="stat-number" style="color:#3b82f6;">${assets.size}</div>
        <div class="stat-note">registered parts</div>
      </div>
      <div class="stat-box">
        <div class="stat-label">Total Scans</div>
        <div class="stat-number">${totalScans}</div>
        <div class="stat-note">${todayScans} today</div>
      </div>
      <div class="stat-box">
        <div class="stat-label">System Status</div>
        <div class="stat-number" style="font-size:14px;padding-top:6px;"><span class="status-online"><span class="dot"></span>Operational</span></div>
        <div class="stat-note">all systems running</div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-header">
        <span class="card-title">Recent Assets</span>
        <a href="/list" class="btn btn-secondary btn-sm">View All</a>
      </div>
      ${recent.length > 0 ? `
      ...
      <div class="empty">${I.box}<p>No assets yet. Create your first QR label.</p>
        <a href="/generate-form" class="btn btn-primary">${I.plus} Generate First Label</a>
      </div>`}
    </div>
  `, 'home')}`);
});
...
app.post('/login', (req, res) => {
  ...
  res.status(401).send(`${HEAD('Login Failed')}<body><div class="auth-wrap">
    <div style="width:100%;max-width:360px;">
      <div class="card card-body">
        <div class="alert alert-error">Invalid credentials. Please try again.</div>
        <a href="/" class="btn btn-secondary btn-block">&larr; Go Back</a>
      </div>
    </div>
  </div></body></html>`);
});
...
app.get('/generate-form', (req, res) => {
  ...
  res.send(`${HEAD('Generate Label')}<body>
  ${LAYOUT(`
    <div class="page-header">
      <div>
        <h1 class="page-title">Generate QR Label</h1>
        <p class="page-sub">Fill in part details to create a new label</p>
      </div>
    </div>
        ...
        <div class="card-body">
          <form action="/generate" method="POST">
            <div class="form-1">
              ...
              <input type="text" name="companyName" placeholder="e.g. AMBER ENTERPRISES INDIA LIMITED" required>
            </div>
            <div class="form-row">
              <div>
                <label>Part No. *</label>
                <input type="text" name="partNo" placeholder="e.g. 93198464460" required>
              </div>
              <div>
                <label>Part Name *</label>
                <input type="text" name="partName" placeholder="e.g. FPE T3 200*100" required>
              </div>
            </div>
            <div class="form-row">
              <div>
                <label>Size</label>
                <input type="text" name="size" placeholder="e.g. 3MMX100MMX200MM">
              </div>
              <div>
                <label>Lot No. *</label>
                <input type="text" name="lotNo" placeholder="e.g. 28032601" required>
              </div>
            </div>
            <div class="form-row">
              <div>
                <label>Quantity *</label>
                <input type="text" name="quantity" placeholder="e.g. 100 Pcs" required>
              </div>
              <div>
                <label>Packer Name *</label>
                <input type="text" name="packer" placeholder="Packer name or ID" required>
              </div>
            </div>
            <div class="form-row" style="margin-bottom:20px;">
              <div>
                <label>Month * <span style="font-weight:400;color:#9ca3af;">(AMI number on label)</span></label>
                <select name="month" required>
                  ${Array.from({ length: 12 }, (_, i) => {
    const m = i + 1;
    const names = ['January','February','March','April','May','June','July','August','September','October','November','December'];
    const sel = m === currentMonth ? 'selected' : '';
    return `<option value="${String(m).padStart(2,'0')}" ${sel}>${String(m).padStart(2,'0')} &mdash; ${names[i]}</option>`;
  }).join('')}
                </select>
              </div>
              <div>
                <label>WO No. * <span style="font-weight:400;color:#9ca3af;">(work order number)</span></label>
                <input type="text" name="woNo" placeholder="e.g. MFG-WO-2026-28032" required>
              </div>
            </div>
...
app.post('/generate', async (req, res) => {
  ...
  if (!companyName || !partNo || !partName || !lotNo || !quantity || !packer || !month || !woNo) {
    return res.status(400).send(`${HEAD('Error')}<body><div class="auth-wrap">
  ...
  if (assets.has(assetKey)) {
    return res.status(400).send(`${HEAD('Duplicate')}<body><div class="auth-wrap">
      <div style="max-width:380px;width:100%;">
        <div class="card card-body">
  ...
  try {
    ...
    ${LAYOUT(`
      <div class="page-header">
        <div>
          <h1 class="page-title">Label Created</h1>
          <p class="page-sub">Your QR label is ready to print</p>
        </div>
      ...
      <div style="overflow-x:auto;">
        <div class="ami-label">
          <div class="ami-label-header">
            <div class="ami-label-company">
              <div style="font-size:9px;font-weight:bold;letter-spacing:0.1em;color:#555;margin-bottom:3px;">COMPANY</div>
              <div class="co-name">${companyName.toUpperCase()}</div>
            </div>
            <div class="ami-label-amino">
              <div class="ami-star">* AMI *</div>
              <div class="ami-num">${String(month).padStart(2,'0')}</div>
            </div>
            <div class="ami-label-rohs">
              <div class="rohs-box">ROHS 2<br>FREE</div>
            </div>
          </div>
          ...
          <div class="ami-label-bottom">
            <div class="ami-qr-cell">
              <div class="qr-label-text">QR CODE</div>
              <img src="${qrDataUrl}" alt="QR Code">
              <div class="qr-scan-text">SCAN FOR DETAILS</div>
            </div>
            <div class="ami-wo-cell" style="justify-content:center;">
              <div>
                <div class="wo-label">WO NO.</div>
                <div class="wo-value">${woNo.trim()}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <p style="font-size:12px;color:#64748b;margin-top:8px;max-width:560px;">
        Click <strong>Print Label</strong> to choose layout, copies, page size, and more.
      </p>
      ...
      ${printModalHTML()}
    `, 'gen')}
    ${printModalScript(qrDataUrl, asset)}`);
  } catch (err) {
    console.error(err);
...
app.get('/list', (req, res) => {
  ...
  res.send(`${HEAD('All Assets')}<body>
  ${LAYOUT(`
    <div class="page-header">
      <div>
        <h1 class="page-title">All Assets</h1>
        <p class="page-sub">${assets.size} total record${assets.size !== 1 ? 's' : ''}</p>
      </div>
      <a href="/generate-form" class="btn btn-primary">${I.plus} New Label</a>
    </div>
    <div class="card">
      ${assets.size === 0
        ? `<div class="empty">${I.box}<p>No assets registered yet.</p><a href="/generate-form" class="btn btn-primary">Generate First Label</a></div>`
...
app.get('/labels', async (req, res) => {
          ...
          <div style="border:2px solid #111;font-family:'Courier New',monospace;font-size:10px;background:#fff;">
            <div style="display:flex;align-items:stretch;border-bottom:1.5px solid #111;">
              <div style="flex:1;padding:5px 8px;border-right:1.5px solid #111;">
                <div style="font-size:7px;font-weight:bold;color:#555;letter-spacing:0.08em;">COMPANY</div>
                <div style="font-size:9px;font-weight:bold;line-height:1.2;">${a.companyName.toUpperCase()}</div>
              </div>
  ...
  res.send(`${HEAD('Labels')}<body>
  ${LAYOUT(`
    <div class="page-header">
      <div>
        <h1 class="page-title">Labels</h1>
        <p class="page-sub">${assets.size} label${assets.size !== 1 ? 's' : ''} stored</p>
      </div>
      <a href="/generate-form" class="btn btn-primary">${I.plus} New Label</a>
    </div>
    ${cards
    ...
    <div style="overflow-x:auto;">
      <div class="ami-label">
        <div class="ami-label-header">
          <div class="ami-label-company">
            <div style="font-size:9px;font-weight:bold;letter-spacing:0.1em;color:#555;margin-bottom:3px;">COMPANY</div>
            <div class="co-name">${asset.companyName.toUpperCase()}</div>
          </div>
          <div class="ami-label-amino">
            <div class="ami-star">* AMI *</div>
            <div class="ami-num">${String(asset.month || '03').padStart(2,'0')}</div>
          </div>
          <div class="ami-label-rohs">
            <div class="rohs-box">ROHS 2<br>FREE</div>
          </div>
        ...
        <div class="ami-label-bottom">
          ...
          <div class="ami-wo-cell" style="justify-content:center;">
            <div>
              <div class="wo-label">WO NO.</div>
              <div class="wo-value">${asset.woNo}</div>
            </div>
          </div>
        </div>
      </div>
...
app.get('/qr/all', async (req, res) => {
  ...
  res.send(`${HEAD('QR Gallery')}<body>
  ${LAYOUT(`
    <div class="page-header">
      <div>
        <h1 class="page-title">QR Gallery</h1>
...
app.get('/asset/:id', async (req, res) => {
  ...
  if (!isAdmin) {
    ...
    asset.scanHistory.push({ timestamp: new Date().toISOString(), device: req.headers['user-agent'] || 'Unknown' });
    await saveAssets(assets);
  }
  ...
  const scanRows = (asset.scanHistory || []).slice(-10).reverse().map(s => `<tr>
  ...
  if (isAdmin) {
    ...
    ${LAYOUT(`
      <div class="page-header">
        <div>
          <h1 class="page-title">${asset.partName}</h1>
          <p class="page-sub">${asset.companyName} &middot; <span class="badge badge-blue">${asset.partNo}</span></p>
        </div>
        <div style="display:flex;gap:8px;">
          <a href="/qr/${req.params.id}/download" class="btn btn-secondary">${I.download} Download QR</a>
      ...
      <div class="grid-aside">
        <div>
          <div class="card">
            <div class="card-header">
              <span class="card-title">Part Details</span>
              <span class="badge badge-${scans > 0 ? 'green' : 'gray'}">${scans} scan${scans !== 1 ? 's' : ''}</span>
            </div>
            <div class="card-body">
              <div class="detail-grid">
                <div style="grid-column:1/-1;">
                  <div class="detail-label">Company Name</div>
                  <div class="detail-value">${asset.companyName}</div>
                </div>
                <div>
                  <div class="detail-label">Part No.</div>
                  <div class="detail-value" style="font-family:monospace;font-size:13px;">${asset.partNo}</div>
                </div>
                <div>
                  <div class="detail-label">Part Name</div>
                  <div class="detail-value">${asset.partName}</div>
                </div>
                <div>
                  <div class="detail-label">Lot No.</div>
                  <div class="detail-value" style="font-family:monospace;font-size:13px;">${asset.lotNo}</div>
                </div>
                <div>
                  <div class="detail-label">Size</div>
                  <div class="detail-value">${asset.size || '&mdash;'}</div>
                </div>
                <div>
                ...
                <div>
                  <div class="detail-label">Packer</div>
                  <div class="detail-value">${asset.packer}</div>
                </div>
                <div>
                  <div class="detail-label">Month (AMI No.)</div>
                  <div class="detail-value">${monthNum}</div>
                </div>
                <div>
                  <div class="detail-label">WO No.</div>
                  <div class="detail-value" style="font-family:monospace;font-size:13px;">${woNum}</div>
                </div>
                <div>
                  <div class="detail-label">Created</div>
                  <div class="detail-value" style="font-size:13px;">${new Date(asset.createdAt).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'})}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card">
            <div class="card-header">
              ...
              <span class="badge badge-green">${scans} total</span>
            </div>
            <div class="table-wrap">
              <table>
                <thead><tr><th>Timestamp</th><th>Device</th></tr></thead>
      ...
      ${printModalHTML()}
    `, 'list')}
    ${printModalScript(printQrDataUrl, asset)}`);
  }
  ...
  const PUBLIC_CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f5f6fa; color: #1a1a1a; }
  .screen-wrap { min-height: 100vh; display: flex; flex-direction: column; align-items: center; padding: 24px 16px 40px; }
  .top-bar { width: 100%; max-width: 600px; display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; }
  .top-brand { font-size: 13px; font-weight: 700; color: #1e293b; display: flex; align-items: center; gap: 8px; }
  .brand-dot { width: 28px; height: 28px; background: #3b82f6; border-radius: 7px; display: flex; align-items: center; justify-content: center; }
  .brand-dot svg { width: 15px; height: 15px; color: white; }
  .scan-badge { background: #f0fdf4; color: #16a34a; border: 1px solid #bbf7d0; border-radius: 99px; font-size: 11px; font-weight: 600; padding: 3px 10px; display: flex; align-items: center; gap: 5px; }
  .scan-dot { width: 6px; height: 6px; background: #22c55e; border-radius: 50%; animation: pulse 2s infinite; }
  @keyframes pulse { 0%,100%{opacity:1}50%{opacity:0.3} }
  .print-fab { display: inline-flex; align-items: center; gap: 7px; padding: 10px 20px; background: #1e293b; color: #fff; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; font-family: inherit; margin-top: 18px; }
  .print-fab svg { width: 15px; height: 15px; }
  .ami-label { width: 100%; max-width: 560px; border: 2.5px solid #111; font-family: 'Courier New', Courier, monospace; background: #fff; color: #111; }
  .ami-label * { box-sizing: border-box; }
  .ami-label-header { display: flex; align-items: stretch; border-bottom: 2px solid #111; }
  .ami-label-company { flex: 1; padding: 8px 14px; border-right: 2px solid #111; }
  ...
  .print-modal-header { padding: 16px 18px; border-bottom: 1px solid #e5e7eb; display: flex; align-items: center; justify-content: space-between; }
  .print-modal-title { font-size: 15px; font-weight: 700; color: #0f172a; }
  .print-modal-close { width: 28px; height: 28px; border-radius: 6px; border: 1px solid #e5e7eb; background: #f9fafb; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 16px; color: #6b7280; }
  .print-modal-close:hover { background: #fee2e2; color: #ef4444; }
  .print-modal-body { padding: 18px; }
  .print-modal-footer { padding: 12px 18px; border-top: 1px solid #e5e7eb; display: flex; gap: 8px; justify-content: flex-end; }
  .print-option-group { margin-bottom: 16px; }
  .print-option-label { font-size: 11px; font-weight: 700; color: #374151; text-transform: uppercase; letter-spacing: 0.07em; margin-bottom: 8px; }
  .print-option-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 6px; }
  .print-option-grid.cols-3 { grid-template-columns: repeat(3,1fr); }
  .print-opt-btn { border: 2px solid #e5e7eb; border-radius: 7px; padding: 8px 6px; text-align: center; cursor: pointer; background: #fff; font-family: inherit; }
  .print-opt-btn.selected { border-color: #3b82f6; background: #eff6ff; color: #1d4ed8; }
  .print-opt-btn .opt-num { font-size: 18px; font-weight: 800; color: #0f172a; line-height: 1; }
  .print-opt-btn.selected .opt-num { color: #1d4ed8; }
  .print-opt-btn .opt-lbl { font-size: 9px; color: #6b7280; font-weight: 600; margin-top: 2px; text-transform: uppercase; }
  .print-opt-btn.selected .opt-lbl { color: #3b82f6; }
  .print-toggle-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f1f5f9; }
  .print-toggle-row:last-child { border-bottom: none; }
  .print-toggle-info .ptl { font-size: 13px; font-weight: 500; color: #111; }
  ...
  .toggle-switch input:checked + .toggle-slider { background: #3b82f6; }
  .toggle-switch input:checked + .toggle-slider:before { transform: translateX(16px); }
  .print-preview-bar { background: #f8fafc; border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px 14px; margin-bottom: 16px; }
  .print-preview-title { font-size: 11px; font-weight: 700; color: #374151; text-transform: uppercase; letter-spacing: 0.07em; margin-bottom: 8px; }
  .btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; border-radius: 6px; font-size: 13px; font-weight: 500; cursor: pointer; text-decoration: none; border: 1px solid transparent; font-family: inherit; }
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${asset.partName} &mdash; AMI Label</title>
...
<body>
  <div class="screen-wrap">
    <div class="top-bar">
      <div class="top-brand">
        <div class="brand-dot"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg></div>
        AMI QR System
      </div>
      <div class="scan-badge"><span class="scan-dot"></span>${scans} scan${scans !== 1 ? 's' : ''}</div>
    </div>

    <div class="ami-label">
      <div class="ami-label-header">
        <div class="ami-label-company">
          <div style="font-size:8px;font-weight:bold;letter-spacing:0.1em;color:#555;margin-bottom:2px;">COMPANY</div>
          <div class="co-name">${asset.companyName.toUpperCase()}</div>
        </div>
        <div class="ami-label-amino">
          <div class="ami-star">* AMI *</div>
          <div class="ami-num">${monthNum}</div>
        </div>
        <div class="ami-label-rohs">
          <div class="rohs-box">ROHS 2<br>FREE</div>
        </div>
      ...
      <div class="ami-label-bottom">
        ...
        <div class="ami-wo-cell">
          <div class="wo-label">WO NO.</div>
          <div class="wo-value">${woNum}</div>
        </div>
      </div>
    </div>

    <button class="print-fab" id="openPrintModal">
  ...
  <div class="print-modal-overlay" id="printModalOverlay">
    <div class="print-modal">
      <div class="print-modal-header">
        <div class="print-modal-title">Print Settings</div>
        <button class="print-modal-close" id="closeModal">&#x2715;</button>
      </div>
      <div class="print-modal-body">
        <div class="print-preview-bar">
          <div class="print-preview-title">Page Layout Preview</div>
          <div id="previewGrid" style="display:grid;gap:3px;max-width:160px;"></div>
          <div id="previewInfo" style="font-size:11px;color:#64748b;margin-top:6px;"></div>
        </div>
        <div class="print-option-group">
          <div class="print-option-label">Labels Per Page</div>
        ...
        <div class="print-option-group">
          ...
          <div id="customCopiesWrap" style="display:none;margin-top:8px;">
            <input type="number" id="customCopiesInput" min="1" max="200" value="10"
        ...
        <div class="print-option-group">
          ...
          <div style="border:1px solid #e5e7eb;border-radius:8px;padding:0 12px;">
            <div class="print-toggle-row">
              <div class="print-toggle-info"><div class="ptl">Show QR Code</div><div class="pts">Include QR code on each label</div></div>
              <label class="toggle-switch"><input type="checkbox" id="togQR" checked onchange="updatePreview()"><span class="toggle-slider"></span></label>
            </div>
            <div class="print-toggle-row">
              <div class="print-toggle-info"><div class="ptl">Show Company Header</div><div class="pts">Print company name and AMI number</div></div>
              <label class="toggle-switch"><input type="checkbox" id="togCompany" checked onchange="updatePreview()"><span class="toggle-slider"></span></label>
            </div>
            <div class="print-toggle-row">
              <div class="print-toggle-info"><div class="ptl">Show Border</div><div class="pts">Bold outer border on each label</div></div>
  ...
  ${LAYOUT(`
    ...
    <div style="max-width:300px;">
      <div class="card" style="text-align:center;">
        <div class="card-body">
          <div class="qr-wrap" style="display:inline-block;margin-bottom:16px;">
